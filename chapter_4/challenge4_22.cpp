...
int main()
{
    ...
    if (package == 'A'  && hours > 10 || package == 'B' && price > PACKAGE_C_MONTHLY_FEE)
    {
        cout << "If you were to upgrade to ";
        switch(package)
        {
            case 'A':
                cout << "Package B, you would save";
                if (hours > PACKAGE_B_CALL_TIME_PROVIDED)
                    newPrice = PACKAGE_B_MONTHLY_FEE + (hours - PACKAGE_B_CALL_TIME_PROVIDED) * PACKAGE_B_ADDITIONAL_USAGE;
                else
                    newPrice = PACKAGE_B_MONTHLY_FEE;
                cout << " $" << (price - newPrice) << ".\n";
                cout << "And if you were to upgrade to ";
            case 'B':
                cout << "Package C, you would save";
                newPrice = PACKAGE_C_MONTHLY_FEE;
                cout << " $" << (price - newPrice) << ".\n";
        }
    }
    return 0;
